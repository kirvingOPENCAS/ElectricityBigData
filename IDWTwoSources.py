...
#base info of hive database
    ...
    while time_end<timeThr:
        ...
        #sentence of get data from weather bureau
        sqlBureau="select site,atmsprs,rain,rhumd,temp,wnddr,wndsp from GDWB where d="+qua_dtThis+" and time>="+qua_time_start+" and time<"+qua_time_end
        ...
        with pyhs2.connect(host=host,
            ...
            #get the sample data for interpolation
                ...
                #get weather bureau data
                cur.execute(sqlBureau)
                bureauDataSet=cur.fetch()
        #get the atmosprsData's site set
        atmosprsSiteSet=[]
        for ele in atmosprsData:
            atmosprsSiteSet.append(ele[0])

        #get the rhumdData's site set
        rhumdSiteSet=[]
        for ele in rhumdData:
            rhumdSiteSet.append(ele[0])

        #get the rainData's site set
        rainSiteSet=[]
        for ele in rainData:
            rainSiteSet.append(ele[0])

        #get the tempData's site set
        tempSiteSet=[]
        for ele in tempData:
            tempSiteSet.append(ele[0])

        #get the wnddrData's site set
        wnddrSiteSet=[]
        for ele in wnddrData:
            wnddrSiteSet.append(ele[0])

        #get the wndspData's site set
        wndspSiteSet=[]
        for ele in wndspData:
            wndspSiteSet.append(ele[0])

        #merge the two dataset which come from south network weather bureau and guangdong weather bureau into a new dataset
        for ele in bureauDataSet:
            #if the site in bureauDataSet doesn't appear in the south network dataset,then append it
            if ele[0] not in atmosprsSiteSet:
                atmosprsData.append([ele[0],ele[1]])
            #if the site in bureauDataSet has already exist in the south network dataset,then compute its average value
            else:
                idx=atmosprsSiteSet.index(ele[0])
                avgVal=(atmosprsData[idx][1]+ele[1])/2
                atmosprsData[idx][1]=avgVal

            if ele[0] not in tempSiteSet:
                tempData.append([ele[0],ele[2]])
            else:
                idx=tempSiteSet.index(ele[0])
                avgVal=(tempData[idx][1]+ele[2])/2
                tempData[idx][1]=avgVal

            if ele[0] not in rhumdSiteSet:
                rhumdData.append([ele[0],ele[3]])
            else:
                idx=rhumdSiteSet.index(ele[0])
                avgVal=(rhumdData[idx][1]+ele[3])/2
                rhumdData[idx][1]=avgVal

            if ele[0] not in rainSiteSet:
                rainData.append([ele[0],ele[4]])
            else:
                idx=rainSiteSet.index(ele[0])
                avgVal=(rainData[idx][1]+ele[4])/2
                rainData[idx][1]=avgVal

            if ele[0] not in wnddrSiteSet:
                wnddrData.append([ele[0],ele[5]])
            else:
                idx=wnddrSiteSet.index(ele[0])
                avgVal=(wnddrData[idx][1]+ele[5])/2
                wnddrData[idx][1]=avgVal

            if ele[0] not in wndspSiteSet:
                wndspData.append([ele[0],ele[6]])
            else:
                idx=wndspSiteSet.index(ele[0])
                avgVal=(wndspData[idx][1]+ele[6])/2
                wndspData[idx][1]=avgVal
